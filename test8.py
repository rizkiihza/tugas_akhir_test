...
def check_size(size):
    if size < MIN_SIZE or size > MAX_SIZE:
        experiment.flag("size < MIN_SIZE or size > MAX_SIZE")
        raise ValueError
    experiment.flag("size >= MIN_SIZE or size <= MAX_SIZE")
    return size
    ...
    for x in range(size):
        for y in range(size):
            if not minefield[x][y] & MINE_FLAG:
                experiment.flag("not minefield[x][y] & MINE_FLAG")
                minefield[x][y] |= calc_cell(size, minefield, x, y)
            else:
                experiment.flag("minefielx[x][y] & MINE_FLAG")
    return
...
def empty_cells(size, minefield, x, y):
    ...
    for (i, j) in list_adjc(size, x, y):
        if minefield[i][j] & MINE_FLAG:
            experiment.flag("minefield[i][j] & MINE_FLAG")
            seed_cell(size, minefield, test)
            minefield[i][j] &= ~MINE_FLAG
        else:
            experiment.flag("not (minefield[i][j] & MINE_FLAG)")
...
def get_coord(c, size):
    ...
    if i < 0 or i >= size:
        experiment.flag("i < 0 or i >= size")
        raise ValueError
    experiment.flag("i >= 0 and i < size")
    return i
...
def format_cell(val, showall):
    if val == 0:
        experiment.flag("val == 0")
        fmt = " "
    elif val < 9:
        experiment.flag("val == %d" % (val))
        fmt = str(val)
    elif showall:
        experiment.flag("showall is True")
        if val & MINE_FLAG:
            experiment.flag("showall is True and val and MINE_FLAG")
            if (val & DEAD_FLAGS) == DEAD_FLAGS:
                experiment.flag("showall is True and val and MINE_FLAG and ((val & DEAD_FLAGS) == DEAD_FLAGS)")
                fmt = "X"
            elif val & MARK_FLAG:
                experiment.flag("showall is True and val and MINE_FLAG and (val & MARK_FLAG)")
                fmt = "#"
            else:
                experiment.flag("showall is True and val and MINE_FLAG and else")
                fmt = "*"
        elif val & MARK_FLAG:
            experiment.flag("showall is True and val and MARK_FLAG")
            fmt = "x"
        else:
            experiment.flag("showall is True and else")
            fmt = "."
    else:
        assert val & HIDE_FLAG
        if val & MARK_FLAG:
            experiment.flag("else and (val & MARK_FLAG)")
            fmt = "#"
        elif val & UNK_FLAG:
            experiment.flag("else and (val & UNK_FLAG)")
            fmt = "?"
        else:
            experiment.flag("else and else")
            fmt = "."
...
def mark_cell(val, req_flags):
    ...
    if req_flags & MARK_FLAG:
        experiment.flag("req_flags & MARK_FLAG")
        if curr_flags == 0:
            experiment.flag("req_flags & MARK_FLAG and curr_flags == 0")
            new_flags = MARK_FLAG
        elif curr_flags & MARK_FLAG:
            experiment.flag("req_flags & MARK_FLAG and curr_flags & MARK_FLAG")
            new_flags = UNK_FLAG
        else:
            experiment.flag("req_flags & MARK_FLAG and else")
            new_flags = 0
    else:
        experiment.flag("not req_flags & MARK_FLAG")
...
def clear_cell(size, minefield, x, y):
    ...
    if dead:
        experiment.flag("dead")
        minefield[x][y] |= DEAD_FLAGS
    else:
        experiment.flag("not dead")

    if not dead and minefield[x][y] == 0:
        experiment.flag("not dead and minefield[x][y] == 0")
